# Specs de Publicação nas Plataformas — Requisitos de API & Formatos de Conteúdo

Este arquivo define os requisitos a nível de API para publicação e gerenciamento de conteúdo nas plataformas de marketing. Use esta referência ao construir payloads, mapear campos e validar conteúdo antes da execução. Para specs de criativo visual (tamanhos de imagem, limites de caracteres para posts orgânicos), consulte `platform-specs.md`.

---

## 1. WordPress

### REST API — Criação de Post
...
### Campos Meta do Yoast SEO

| Chave Meta | Tamanho Máx. | Finalidade |
|---|---|---|
...
| Parâmetro | Valor |
|---|---|
| Autenticação | Application Passwords (recommended), JWT, or OAuth 2.0 |
| Rate limit | No built-in limit; hosting provider may impose limits (typically 60-120 req/min) |
| Upload máx. de mídia | Determined by server `upload_max_filesize` (typically 2-64 MB) |
| Operações em lote | Not natively supported; use sequential requests with 100ms delay |

---

## 2. Webflow

### CMS API — Criação de Item
...
| Parâmetro | Valor |
|---|---|
| API Geral | 60 requests per minute per site |
| CMS API | 60 requests per minute |
| API de Publicação | 1 publish per minute per site |
| Autenticação | API token (v2) or OAuth 2.0 |
| Máx. de itens por coleção | 10,000 (CMS plan), 100,000 (Business plan) |

---

## 3. Plataformas de E-mail

### Comparação de Payload

| Funcionalidade | SendGrid | Klaviyo | Customer.io | Brevo (Sendinblue) | Mailgun |
|---|---|---|---|---|---|
...
| Plataforma | Método de Autenticação |
|---|---|
...
### Hierarquia de Campanha e Campos

| Nível | Campos Principais | Detalhes |
|---|---|---|
...
### Campos do Anúncio de Pesquisa Responsivo (RSA)

| Campo | Qtd. | Tamanho Máx. | Notas |
|---|---|---|---|
...
| Estratégia | Quando Usar | Parâmetro Chave |
|---|---|---|
...
### Conteúdo Patrocinado — Campos de Criativo

| Campo | Tamanho Máx. |
|---|---|
| Texto introdutório | 600 chars (150 recommended for no truncation) |
| Headline | 200 chars (70 recommended) |
| Descrição | 300 chars (100 recommended) |
| Imagem | 1200 x 627 px (1.91:1). Also supports 1080x1080 and 1080x1350. Max 5 MB. |
| Vídeo | 3s-30min. 360p-1080p. MP4. Max 200 MB. |
...
| Campo | Especificação |
|---|---|
| Proporção do vídeo | 9:16 (recommended), 1:1, 16:9 |
| Duração do vídeo | 5-60 seconds (9-15 seconds recommended for In-Feed) |
| Resolução do vídeo | 720x1280 minimum (1080x1920 recommended) |
| Formato do vídeo | MP4, MOV, MPEG, AVI |
| Tamanho do arquivo de vídeo | Max 500 MB |
| Texto do anúncio | 1-100 characters (emoji supported) |
| Nome de exibição | Max 40 characters |
| Imagem de perfil | 50x50 px minimum |

---

## 8. Specs de Post Social — Referência Rápida

| Plataforma | Máx. Chars | Tamanho de Imagem | Duração Máx. de Vídeo | Tamanho Máx. de Vídeo | Hashtags | Comportamento de Link |
|---|---|---|---|---|---|---|
...
### Especificações de SMS

| Parâmetro | Valor |
|---|---|
| Codificação GSM-7 | 160 characters per segment. Characters: A-Z, a-z, 0-9, standard punctuation. |
| Codificação Unicode | 70 characters per segment. Triggered by: emojis, non-Latin scripts, special characters. |
| SMS concatenado | Up to 10 segments (1,600 GSM-7 chars or 700 Unicode chars). Each segment billed separately. |
| Mídia MMS | Max 5 MB per message. Supported: JPEG, PNG, GIF, MP4, MP3. |
| Tipos de remetente (EUA) | Toll-free (verified), short code (high throughput, $500-$1,000/month), 10DLC (registered, $2-$15/month A2P fee). |
| Tipos de remetente (Internacional) | Alphanumeric sender ID (11 chars, e.g., "ACME" — not available in US/Canada). |
| Throughput | Toll-free: 40 MPS. Short code: 100+ MPS. 10DLC: 1-75 MPS (based on trust score). |
| Conteúdo obrigatório | Opt-out instructions in every marketing message: "Reply STOP to unsubscribe" or equivalent. |
| Horário de silêncio | No sends 9 PM-8 AM recipient local time (industry best practice; some states legally mandate). |

### WhatsApp Business API

| Parâmetro | Valor |
|---|---|
| Tipos de mensagem | Template messages (pre-approved, for initiating conversations) and session messages (within 24h window after user message). |
| Aprovação de template | Submit via Meta Business Manager. Review: 24-48 hours. Categories: MARKETING, UTILITY, AUTHENTICATION. |
| Variáveis de template | `{{1}}`, `{{2}}`, etc. Positional placeholders. Up to 10 per template. |
| Limite de mensagem de texto | 1,024 characters. |
| Tipos de mídia | Image (5 MB), Video (16 MB), Document (100 MB), Audio (16 MB), Sticker (100 KB static, 500 KB animated). |
| Mensagens interativas | Buttons (up to 3 quick-reply buttons, 20 chars each) or lists (up to 10 items in up to 10 sections). |
| Precificação de conversa | Meta charges per 24-hour conversation window. Rates vary by country and conversation category. Marketing conversations: ~$0.02-$0.08 USD (varies by country). |
| Classificação de qualidade | Green (high), Yellow (medium), Red (low). Low quality = reduced throughput or template rejection. Maintain by keeping opt-out and block rates low. |
| Requisito de opt-out | Must provide opt-out mechanism. High block rates trigger quality rating drops. Honor opt-outs within 24 hours. |
